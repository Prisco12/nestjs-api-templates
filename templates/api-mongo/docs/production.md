# Produção segura e simples — MongoDB

Este guia parte do cenário em que somente a pasta `api-mongo` foi copiada para um projeto novo. O objetivo é publicar apenas a API por HTTPS e manter banco, Redis e observabilidade fora da internet.
...
- MongoDB, Redis, Loki, Tempo, Prometheus e Alloy existem somente na rede interna.
...
- `MONGO_USERNAME`, `MONGO_PASSWORD` e `REDIS_PASSWORD`;
...
Não use `down -v` em produção: essa opção remove os volumes de MongoDB, Redis, Grafana, Loki, Tempo e Prometheus.

...
export class EmailService {
  ...
  private sendAccountEmail(input: {
    ...
    const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;padding:24px"><h1 style="font-size:24px">${input.title}</h1><p>${input.message}</p><p style="margin:32px 0"><a href="${link}" style="background:#2563eb;color:#fff;padding:12px 20px;text-decoration:none;border-radius:6px">${input.button}</a></p><p style="color:#475569">${input.validity}</p><p style="font-size:12px;color:#64748b;word-break:break-all">${link}</p></div>`;

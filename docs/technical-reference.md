...
Erros de validação acrescentam detalhes estruturados por campo:
...
`field` identifica o input, `code` é estável para tradução no cliente e `message` serve como fallback. A política compartilhada por cadastro e redefinição exige de 12 a 128 caracteres, minúscula, maiúscula, número e símbolo.
...
| `POST /auth/register`               | pública         | Body obrigatório: `email` e senha de 12 a 128 caracteres, com minúscula, maiúscula, número e símbolo. Cria usuário com role `user` e envia confirmação. |
...
| `POST /auth/reset-password`         | pública         | Body: `token`, `password`. Aplica a mesma política do cadastro, troca a senha e revoga todas as sessões.                                                |
